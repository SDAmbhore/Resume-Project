...
connection = pymysql.connect(host='localhost', user='root', db='cv')
...
def safe_literal_eval(value):
    try:
        ...
        try:
            evaluated_value = ast.literal_eval(decoded_value)
...
def run():
    ...
    link = '[©Developed by Sudhanshu Ambhore](https://www.linkedin.com/in/sudhanshu-ambhore-ab5bb3207/)'
    ...
    table_sql = "CREATE TABLE IF NOT EXISTS " + DB_table_name + """
                    (ID INT NOT NULL AUTO_INCREMENT,
                     ...
                     PRIMARY KEY (ID),
                     CONSTRAINT UNIQUE (Name, Email_ID));
                    """
    cursor.execute(table_sql)
    if choice == 'User':
        st.markdown('''<h5 style='text-align: left; color: yellow;'> Upload your resume, and get smart 
        recommendations</h5>''',
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                if 'Objective' in resume_text:
                    ...
                    st.markdown('''<h5 style='text-align: left; color: #48C9B0;
                    '>[+] Awesome! You have added Objective</h4>''', unsafe_allow_html=True)
                else:
                    st.markdown('''<h5 style='text-align: left; color: #CD6155;
        ...
        if st.button('Login'):
            if ad_user == 'sudhanshu' and ad_password == 'sudhanshu1404':
                st.success("Welcome Sudhanshu Ambhore!")
                # Display Data
                ...
                df['Predicted Field'] = df['Predicted Field'].apply(safe_literal_eval)
                df['User Level'] = df['User Level'].apply(safe_literal_eval)
                df['Actual Skills'] = df['Actual Skills'].apply(safe_literal_eval)
                df['Recommended Skills'] = df['Recommended Skills'].apply(safe_literal_eval)
                df['Recommended Course'] = df['Recommended Course'].astype(str)
                df['Recommended Course'] = df['Recommended Course'].apply(safe_literal_eval)
                st.dataframe(df)
                ...
                predicted_field_counts = plot_data[
                    'Predicted_Field'].value_counts().reset_index().dropna().drop_duplicates()
                predicted_field_counts.columns = ['Predicted_Field', 'count']
                predicted_field_counts['Predicted_Field'] = predicted_field_counts['Predicted_Field'].apply(
                    lambda x: x.decode('utf-8') if isinstance(x, bytes) else x)
                st.subheader("**Pie-Chart for Predicted Field Recommendation**")
                fig = px.pie(predicted_field_counts, values='count', names='Predicted_Field',
                ...
                fig = px.pie(user_level_counts, values='count', names='User_level',
